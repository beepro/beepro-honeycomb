...
exports.default = function (app, mongoose) {
  ...
  app.use('/api/honeys/:id/files/', upload.single('file'), function (req, res) {
    ...
    if (req.params.id.match('\\.\\./') || relativePath.match('\\.\\./')) {
      res.status(400).send('');
      return;
    }
    var to = _path2.default.join(honeyPath, relativePath);
    if (req.method === 'POST') {
      _fsExtra2.default.ensureDirSync(_path2.default.dirname(to));
...
var SECRET = process.env.BEEPRO_HASH_SECRET || 'AWESOME BEEPRO';
