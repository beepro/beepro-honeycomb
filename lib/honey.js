...
function wsUrl() {
  var host = process.env.GLOBAL_HOST || 'localhost';
  var port = process.env.GLOBAL_PORT || '5432';
  var protocol = void 0;
  if (port === '443') {
    protocol = 'wss';
  } else {
...
function dance(_ref4) {
  ...
  return new Promise(function (resolve) {
    ...
    if (to) {
      fileTo = _path2.default.join(honey.path, to);
    }
    if (type === 'create') {
      _fsExtra2.default.outputFileSync(file, contents, 'utf8');
    }
    if (type === 'delete' && _fsExtra2.default.existsSync(file)) {
      _fsExtra2.default.removeSync(file);
    }
    if (type === 'move' && _fsExtra2.default.existsSync(file)) {
      _fsExtra2.default.moveSync(file, fileTo);
    }
...
function changeUpstream(honey) {
  ...
  return (0, _gitPromise2.default)('add .', { cwd: honey.path }).then(function () {
    return (0, _gitPromise2.default)('status -s', { cwd: honey.path });
  }).then(function (res) {
    return res ? (0, _gitPromise2.default)('commit -m "' + message + '"', { cwd: honey.path }).then(function () {
